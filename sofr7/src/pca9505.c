...
static uint8_t pca9505_read_port(uint8_t addr, uint8_t port) {
    uint8_t data = 0xFF;
    if (i2c_read_register(addr, PCA9505_INPUT_PORT0 + port, &data, 1, 100) != I2C_STATUS_SUCCESS) {
...
void matrix_init_custom(void) {
    ...
    // Initialize each PCA9505 chip
    if (!pca9505_init_chip(0x20)) print("Failed to init chip 0x20\n");
    if (!pca9505_init_chip(0x21)) print("Failed to init chip 0x21\n");
